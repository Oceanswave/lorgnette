...
class PluralsightController {
    ...
    *getNextCourse(ps, db, isStarting) {
        ...
        if (this._opts.playlist) {
            ...
            //RuhRoh.
            if (!selectedPlaylist)
                throw "The specified playlist could not be found: " + this._opts.playlist;
            ...
            if (courses && courses.length > 0) {
                ...
                //If Fresh is specified, favor watching unwatched coureses.
                if (this._opts.fresh) {
                    let watchedCourses = yield db.getWatchedCoursesAsync();
                    let availableCourses = _.filter(courses, function (course) {
                        return !_.some(watchedCourses.courses, {courseName: course.courseName});
                    });

                    console.log("%s courses of %s for the term '%s' have been watched.", courses.length - availableCourses.length, courses.length, this._opts.search);
                    ...
                    if (availableCourses.length === 0) {
                        ...
                        //Otherwise just get a course from all courses retrieved.
                        else {
                        ...
                        course = _.sample(availableCourses);
                    }
                }
                else {
                    course = _.sample(courses);
                ...
                course = yield db.getCourseByIdAsync(course.courseName);
                if (!course) {
                    yield this.ensureCourseListingIsCurrent(ps, db, true);
                    course = yield db.getCourseByIdAsync(course.courseName);
                }
