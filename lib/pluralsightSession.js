...
class PluralsightSession {
    ...
    *watchCourseAsync(course, monitorCallback) {
        ...
        do {
            ...
            if (currentStatus.hasIssueLoadingVideo) {
                try {
                    yield this.reloadAfterHasIssueLoadingVideo();
                    ...
                    debug("An error occurred while attempting reload after an issue loading video..." + ex);
                }
            }

            if (currentStatus.hasLoadingFailure) {
                try {
                    yield this.reloadAfterLoadingFailure();
                } catch(ex) {
                    debug("An error occurred while attempting reload after a loading failure..." + ex);
                }
            }
    ...
    *getCurrentVideoStatusAsync() {
        ...
        var status = yield this._nightmare.evaluate(function () {
            ...
            var hasIssueLoadingVideo = jQuery("#try-again-error").is(":visible");
            var hasLoadingFailure = jQuery("#loading-failure").is(":visible");
            ...
            return {
                ...
                hasIssueLoadingVideo: hasIssueLoadingVideo,
    ...
    *reloadAfterHasIssueLoadingVideo() {
        debug("reloadAfterHasIssueLoadingVideo() starting...");
        
         yield this._nightmare.expectNavigation(function () {
            return this.click("#try-again-error .button-container .button");
        }, 10000);
