...
Nightmare.action('hasJQuery', function () {
    return this.evaluate_now(function () {
        return typeof(jQuery) === "undefined";
    });
});

Nightmare.action('getJQueryVersion', function () {
    return this.evaluate_now(function () {
...
class PluralsightSession {
    ...
    *ensureJQueryAsync() {
        ...
        var jQueryVersion = yield this._nightmare.chain()
            .getJQueryVersion();

        if (!jQueryVersion) {
            debug("ensureJQuery() jQuery not found - injecting jQuery.");
            jQueryVersion = yield this._nightmare.chain()
    ...
    *getCoursesAsync() {
        ...
        var result = yield this._nightmare.chain().wait("#ps-main ol.courses-list li")
            .evaluate(function () {
    ...
    *watchCourseAsync(course, monitorCallback) {
        ...
        var url = yield this._nightmare.url();
        if (!/https:\/\/app.pluralsight.com\/player.*/.test(url)) {
    ...
    *gotoDashboard() {
        ...
        yield this._nightmare.chain().goto("https://app.pluralsight.com/library");
